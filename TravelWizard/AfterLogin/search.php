...
if (!empty($searchTerm)) {
    try {
        ...
        if ($result) {
            // Loop through each matching package
            foreach ($result as $row) {
                $packageID = $row['packageID'];

                // Get destination names and country for the current package
                $destQuery = "
                ...
                foreach ($destinations as $dest) {
                    ...
                    if (empty($country)) {
                        // Use first country found
                        $country = $dest['country'];
                    }
                }

                // Dynamically construct the image path based on the first destination name
                $imageName = strtolower(str_replace(' ', '_', $destinationNames[0])) . '.png';
                $imagePath = "images/destinations/$imageName";
                if (!file_exists($imagePath)) {
                    $imagePath = "images/default.jpg";
                }

                // Generate the destination page URL based on country
