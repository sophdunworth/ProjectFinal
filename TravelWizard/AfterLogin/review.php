...
// Sanitize input  by converting special characters to HTML entities
function escape($data) {
    return htmlspecialchars(($data), ENT_QUOTES, 'UTF-8');
}

// Initialize variables for showing a popup message to the user after form submission
$showPopup = false;
$popupMessage = '';
...
if (isset($_POST['submit'])) {
    try {
        // Collect and sanitize user input from the form
        $new_review = array(
        ...
        if (!in_array('', $new_review, true)) {
            // Construct a secure SQL statement using named parameters to safeguard against SQL injection
            $sql = sprintf(
...
// Display modal popup on page load if a message is set from the server
<?php if ($showPopup): ?>
